...
pub fn max_scroll(area: Rect) -> u16 {
    crate::tui::paragraph_max_scroll(&build_lines(), area)
}
...
fn build_lines() -> Vec<Line<'static>> {
    vec![
        ...
        kv(
            "Home",
            "overall health summary with sources, rules, violations, and significant debt",
        ),
        kv(
            "Sources",
            "internal dependency sources plus handpicked personal and team sources",
        ),
        kv("Rules", "approved rules with file and source detail"),
        kv("Violations", "violation list + config issues"),
        kv(
            "Debt",
            "ranked hotspot triage for dead/dup/dep/hotspot findings",
        ),
        kv(
            "Result",
            "generic command-result view for actions without a dedicated screen",
        ),
    ]
}
