...
# command with a different --schedule.
#
# This is the freshness/drift gate: the `wh ci` step below FAILS the run on
# content-hash or version drift (docs changed since rules were authored).
# Rule ENFORCEMENT itself runs agent-free through the native configs Whetstone
# generates (`wh actions lint` -> ruff/biome/clippy [lints]) in your existing
# lint CI -- Whetstone decides the rules; your normal linters enforce them.
